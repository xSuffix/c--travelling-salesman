                ...
                if (read > 0)
                {
                    ...
                    for (int j = 0; j < distanceTable.n; j++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                setConsoleColor(COLOR_ERROR);
                                printf("Error reading from file: Expected number greater than 0, got '%s' in line %u "
                                       "word %u.\n",
                                       distString, i + 2, j + 1);
                                i = j = distanceTable.n;
                            }
...
void showData(DistanceTable distanceTable) // TODO Print if there are unsaved changes
{
    if (distanceTable.n)
    {
        ...
        {
            printf("[%2d] - from: %d | to: %d | dist: %3d\n", i, distanceTable.distances[i].from,
                   distanceTable.distances[i].to, distanceTable.distances[i].dist);
        }
    }
    else
    {
...
int main()
{
    ...
    Menu startMenu[] = {
        {'a', "Entfernungstabelle laden"},    {'b', "Entfernungstabelle speichern"},
        {'c', "Entfernungstabelle anzeigen"}, {'d', "Entfernung zwischen zwei Städten ändern"},
        {'e', "Kürzeste Route berechnen"},    {'f', "Programm beenden"},
    };
